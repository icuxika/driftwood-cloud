...
import {ApiData, BaseEntity, BindOneDTO, HasId, Page, Pageable} from "../../index";
...
type CreateService = (path: string) => {
     ...
     * 用户分页查询
     */
    page: (pageable: Pageable | User) => Promise<AxiosResponse<ApiData<Page<User>>>>;
     ...
     * 新增用户
     */
    save: (user: Omit<Partial<User>, "id">) => Promise<AxiosResponse<ApiData<never>>>;
     ...
     * 为用户绑定角色
     */
    bindRoles: (bindOneDTO: BindOneDTO) => Promise<AxiosResponse<ApiData<never>>>;
}
